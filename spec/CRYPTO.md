...
Twofish was an **AES finalist** with public analysis; **NESSIE** selected Twofish; CTR mode with an independent Poly1305 key parallels the Serpent profile (Section 6.3 of `spec/CESS-v0.2.md`). **Known-answer** material for **Twofish** and **cascade** rows (`suite_id` in `ALGORITHM-REGISTRY.md`) is in **`vectors/twofish.toml`**; the runner crate `cess_runner::twofish_bulk` checks those vectors in **`cargo test`** (`runner/`). That TOML set covers **CTR + Poly1305** and **registered cascades** in the CESS profile (`cess-twofish-v0.2`); it is **not** a port of the designers’ **Appendix B** specification-chain corpus (~1203 vectors including Monte Carlo), which validates **raw block cipher** traces. Implementations may use both: for example **Galdralag-firmware** carries **`twofish_vectors.json`** for the appendix-style corpus—orthogonal coverage, not interchangeable with **`twofish.toml`**.

**Camellia** (RFC 3713) uses the same CTR + Poly1305 framing for **approved** **`suite_id`** rows (**`0x0031`–`0x0036`**, **`0x0208`–`0x020c`**); **KATs** are in **`vectors/camellia.toml`** and **`cess_runner::camellia_bulk`**. That file also carries **RFC 3713 Appendix A** single-block **ECB** vectors (`suite_id` **`rfc3713-a*`**)—**raw cipher** conformance only, parallel in role to externally maintained **Twofish** appendix-style corpora. **CTR+Poly1305 integration** rows and **ECB** appendix rows address different failure modes and are **not** interchangeable substitutes.
