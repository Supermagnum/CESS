...
**Camellia-128/192/256-CTR + Poly1305:** Camellia uses a **128-bit** block and a **128-, 192-, or 256-bit** key per **`suite_id`** in **`ALGORITHM-REGISTRY.md`**; **CTR** mode with the same **16-byte** counter schedule as **Serpent** and **Twofish** bulk rows in the conformance runner; Poly1305 uses **32-byte** one-time key and the same RFC 8439 MAC data layout as **`vectors/bulk_aead.toml`**. **Known-answer** ciphertexts for the **CTR+Poly1305** integration profile (single-layer, ChaCha/Serpent/Camellia cascades where registered, and **Ed25519** inner-bulk templates) are in **`vectors/camellia.toml`** (`schema` `cess-camellia-v0.2`), along with **informative RFC 3713 Appendix A** single-block **ECB** rows (`suite_id` **`rfc3713-a128`**, **`rfc3713-a192`**, **`rfc3713-a256`**); the latter are **raw cipher** checks only—see **`spec/CRYPTO.md`** and **`vectors/README.md`** for scope.
...
**Informative (classical combination count):** For **PQ-free** inner profiles, **two** classical **Brainpool** curves, **seven** bulk **AEAD** or **cascade** options, **optional** keyed **BLAKE3** integrity, and **optional** **Ed25519** signature **enumerate** **56** **combinatorial** profiles (**2 × 7 × 2 × 2**). **`0x0000`** is a **reserved sentinel** and is **excluded** from the **56**-combination classical product. **`ALGORITHM-REGISTRY.md`** assigns **56** **`suite_id`** values **covering** that **full** product (**`0x0001`–`0x0030`**, **`0x0200`–`0x0207`**), **many** with **provisional** status **until** vector **coverage**; **`vectors/classical_suite_id_matrix.toml`** lists the **provisional** **`suite_id`** values (**`0x0008`–`0x000f`**, **`0x0013`–`0x0030`**) pending **KAT** admission per **registry** **criteria** **6**.
