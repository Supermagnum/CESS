...
Normative detail: `spec/CESS-v0.2.md` Section 12.

When **Mode A — fixed outer wrapper** is used, **BrainpoolP384r1** ECDH **only** establishes `K_outer` for the profile-independent outer layer; see Section 12 of the same document.

Runner-focused notes (inner cascade KATs, HKDF-BLAKE3 labels shared with Galdralag `cess`): see [`docs/CESS_CONFORMANCE.md`](docs/CESS_CONFORMANCE.md).
...
4. Publish a short **conformance statement** naming the CESS version (e.g. `0.2-draft`), level, and commit hash of vectors used.

## Required vectors per level

- **CESS-CORE:** `sss.toml`, `argon2id.toml`, `blake3.toml`, `hkdf_blake3.toml`, `ecdh_brainpool.toml`, `bulk_aead.toml` (ChaCha rows), `inner_cascade.toml` (Galdralag-aligned HKDF-BLAKE3 + ChaCha + inter-layer HMAC-BLAKE3 + Serpent CTR body; see `docs/CESS_CONFORMANCE.md`), `twofish.toml` (Twofish-256-CTR + Poly1305 and registered cascades per `ALGORITHM-REGISTRY.md`), `camellia.toml` (Camellia-128/192/256-CTR + Poly1305, Camellia cascades, and Ed25519 inner-bulk templates for **`0x0031`–`0x0036`** / **`0x0208`–`0x020c`**), `pin_wrap.toml` (ChaCha row), `rejection.toml`, `integration.toml` (ChaCha-only integration rows), `wycheproof_chacha.toml` (subset; full Wycheproof coverage is via `wycheproof_chacha` + JSON), `rfc6932_brainpool.toml` (RFC 6932 IKE ECDH excerpts; full JSON + `scripts/rfc6932_brainpool.py`), `rfc7027_brainpool.toml` (RFC 7027 TLS ECDH excerpts; full JSON + `scripts/rfc7027_brainpool.py`), `rfc5639_brainpool.toml` (RFC 5639 domain-parameter excerpts; JSON + `scripts/rfc5639_brainpool_domain_parameters.py`).  
...
- **CESS-PQ:** all FULL vectors plus PQ-specific tests when published (see `spec/CESS-v0.2.md` Section 7).
...
**CESS** is used as a **descriptive** project name. There is **no trademark grant** in the specification or licences. You MAY say “compatible with CESS v0.2” if your statement is accurate and backed by tests. Do not imply endorsement by contributors or maintainers without their written permission.
