...
**Combinatorial coverage (informative):** Every **56**-cell classical combination has a **`suite_id`** row below. **`vectors/classical_suite_id_matrix.toml`** lists per-row **status**, vector file pointers, and **`pending_issue`** where KAT coverage is still incomplete. **Twofish** inner bulk for **`0x0004`–`0x0007`** and **`0x0203`–`0x0207`** is covered by **`vectors/twofish.toml`** (`cess_runner::twofish_bulk`), which exercises **CESS bulk/cascade** CTR+Poly1305 wiring only—not the full Schneier et al. **Appendix B** block-cipher chain corpus (~1203 vectors; kept separately, e.g. **Galdralag-firmware** `twofish_vectors.json`). **Camellia** inner bulk and **Camellia**-involving cascades for **`0x0031`–`0x0036`** and **`0x0208`–`0x020c`** are covered by **`vectors/camellia.toml`** (`cess_runner::camellia_bulk`) as **CTR+Poly1305** integration KATs; the same file also holds **RFC 3713 Appendix A** single-block **ECB** rows (`suite_id` **`rfc3713-a128`**, **`rfc3713-a192`**, **`rfc3713-a256`**), which validate the **raw block cipher** only and neither replace nor duplicate the hexadecimal **`suite_id`** rows. **Keyed BLAKE3 integrity** has sample KATs in **`vectors/blake3_integrity.toml`** (not yet one row per **`suite_id`** in **`0x0008`–`0x000f`**). **Ed25519** has sample KATs in **`vectors/ed25519_signing.toml`**. **BrainpoolP512r1** inner ECDH + HKDF sample material for **`0x0013`** is in **`vectors/ecdh_p512_inner.toml`** (Python **`scripts/verify_p512_ecdh_kat.py`** cross-checks ECDH). Rows without matching KAT entries remain **provisional** per admission criteria **6** until extended.
...
| 2026-05-05 | **Informative:** **`vectors/twofish.toml`** covers **CESS** CTR+Poly1305 bulk/cascade wiring only (**`cess_runner::twofish_bulk`**); designers’ Appendix B-scale **raw block** corpora (**e.g.** **Galdralag-firmware** **`twofish_vectors.json`**) are **orthogonal** proof assets. |
| 2026-05-05 | **`vectors/camellia.toml`**: **RFC 3713 Appendix A** ECB rows (`suite_id` **`rfc3713-a*`**), distinct from registry **CTR+Poly1305** **`suite_id`** hex rows (**`cess_runner::camellia_bulk`**). |
