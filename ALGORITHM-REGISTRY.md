...
| Integrity (standalone, inner profile) | BLAKE3 | NCC Group, Kudelski | Keyed **32-byte** tags between cascade layers or before signing; HKDF `info` per `spec/CESS-v0.2.md` Section 8.3 |
| Digital signature (inner profile) | Ed25519 | RFC 8032; Bernstein et al.; independent cryptanalysis | Normative when listed in lookup table (Section 4.5); long-term keys out of band |
...
| AEAD (alt-2) | Twofish-256-CTR + Poly1305 | NESSIE; AES competition public analysis | `vectors/twofish.toml`; cascade-capable; no NSA design input |
...
| `0x0004` | BrainpoolP384r1 | — | Twofish-256-CTR + Poly1305 | — | Single-layer Twofish (Section 6.3); `vectors/twofish.toml`. |
| `0x0005` | BrainpoolP384r1 | — | Cascade: ChaCha inner, Twofish outer | — | `vectors/twofish.toml`; Section 4.4. |
| `0x0006` | BrainpoolP384r1 | — | Cascade: Twofish inner, Serpent outer | — | `vectors/twofish.toml`; Section 4.4. |
| `0x0007` | BrainpoolP384r1 | — | Triple cascade: ChaCha inner, Serpent middle, Twofish outer | — | `vectors/twofish.toml`; Section 13.8. |
...
| `0x0200` | BrainpoolP384r1 | — | ChaCha20-Poly1305 | Ed25519 | Signed variant of `0x0001` (Section 4.5). |
| `0x0201` | BrainpoolP384r1 | — | Serpent-256-CTR + Poly1305 | Ed25519 | Signed variant of `0x0002`. |
| `0x0202` | BrainpoolP384r1 | — | Cascade: ChaCha inner, Serpent outer | Ed25519 | Signed variant of `0x0003`. |
| `0x0203` | BrainpoolP384r1 | — | Twofish-256-CTR + Poly1305 | Ed25519 | Signed variant of `0x0004`; inner bulk KAT in `vectors/twofish.toml` (`suite_id` `0x0203`). |
| `0x0204` | BrainpoolP384r1 | — | Cascade: ChaCha inner, Twofish outer | Ed25519 | Signed variant of `0x0005`; inner bulk KAT in `vectors/twofish.toml` (`suite_id` `0x0204`). |
| `0x0205` | BrainpoolP384r1 | — | Cascade: Twofish inner, Serpent outer | Ed25519 | Signed variant of `0x0006`; inner bulk KAT in `vectors/twofish.toml` (`suite_id` `0x0205`). |
| `0x0206` | BrainpoolP384r1 | — | Triple cascade: ChaCha, Serpent, Twofish | Ed25519 | Signed variant of `0x0007`; inner bulk KAT in `vectors/twofish.toml` (`suite_id` `0x0206`). |
| `0x0207` | BrainpoolP384r1 | — | Twofish single + optional keyed BLAKE3 integrity before sign | Ed25519 | **Twofish + BLAKE3 integrity + Ed25519** (Section 6.3); inner bulk KAT in `vectors/twofish.toml` (`suite_id` `0x0207`; includes `blake3_integrity_key_hex`). |
...
**Informative (registry maintenance):** **Unallocated** values include gaps not listed above (for example **`0x0008`–`0x000F`**, **`0x0013`–`0x00FF`** excluding listed rows, **`0x0103`–`0x010F`**, **`0x0113`–`0x011F`**, **`0x0123`–`0x01FF`**, **`0x0208`–`0x02FF`**, **`0x0300`–`0xFFFF`** until allocated).
